...
export function cjsResolve(fs, fragment, parentURL) {
...
// LOAD_AS_FILE(X)
function loadAsFile(fs, fragment, parentURL) {
...
// LOAD_INDEX(X)
function loadIndex(fs, fragment, parentURL) {
...
// LOAD_AS_DIRECTORY(X)
function loadAsDirectory(fs, path) {
...
function loadWithFormat(fs, resolved) {
...
function loadNodeModules(fs, fragment, parentURL) {
...
function loadPackageImports(fs, fragment, parentURL) {
...
function loadPackageExports(fs, fragment, parentURL) {
...
function loadPackageSelf(fs, fragment, parentURL) {
    // 1. Find the closest package scope SCOPE to DIR.
    const packageURL = lookupPackageScope(fs, parentURL);
    // 2. If no scope was found, return.
    if (packageURL === null) {
...
// as-is.
function verbatimFileURLToPath(url) {
    return `${url.pathname}${url.search}${url.hash}`;
}
//# sourceMappingURL=cjsResolve.js.map